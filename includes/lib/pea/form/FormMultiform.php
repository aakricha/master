	...
	*/
	...
	function getRollUpdateSQL($i='')
	{
		if ($i == '' && !is_numeric($i))
		{
			...
			$ids   = $_POST[$this->formName.'_'.$this->referenceField['tbl_id']];
			foreach ($ids as $j => $id)
			{
	...
	function getOutput( $objects = '', $str_name = '', $str_extra = '' )
	{
		...
		{
			$out	= array('<input type="hidden" sytle="display: none;" name="'.$this->formName.'_'.$this->referenceField['tbl_id'].'[]" value="'.$id.'" />');
			foreach ( $this->elements as $id => $input )
			{
				$object    = !empty($objects[$id]) ? $objects[$id] : new stdClass();
