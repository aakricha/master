...
EOT;
	...
	function actionOnDelete()
	{
		...
		if (!empty($this->onDelete))
		{
			...
			$tmp = call_user_func($this->onDelete, $this->onDeleteArgs);
	...
	function actionOnSave()
	{
		...
		{
			$tmp = call_user_func($this->onSave, $this->onSaveArgs);
			if (is_bool($tmp) || $tmp=='1' || $tmp=='0')
			{
				$out = $tmp ? true : false;
			}
	...
	function action()
	{
		...
		if ($this->isActionExecute)
		{
			...
			{
				$this->actionSecurity();
				if (!$this->onDeleteLoadLast)
				{
					$this->error = !$this->actionOnDelete();
				}
				if ($this->isActionExecute)
				{
					$ok = false;
